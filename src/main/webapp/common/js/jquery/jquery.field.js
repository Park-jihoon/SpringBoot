...
(function ($) {
    ...
    $.fn.limitSelection = function (limit, options) {
        ...
        var opt = jQuery.extend(
            (limit && limit.constructor == Object ? limit : {
                limit: limit
                , onsuccess: function (limit) {
                    return true;
                }
                , onfailure: function (limit) {
                    alert("You can only select a maximum a of " + limit + " items.");
                    return false;
                }
            })
            , options);
    ...
    $.fn.createCheckboxRange = function (callback) {
        ...
        if (bCallback)
            this.each(function () {
                opt.click.apply(this, [$.event.fix({type: null}), $(this).is(":checked")])
            });
        ...
        return this.each(
            function () {
                ...
                var checkboxClicked = function (e) {
                    ...
                    if (bCallback) $(this).each(function () {
                        opt.click.apply(this, [e, bSetChecked])
                    });
